...
@cache(expire=60)
def get_workorder(base_id: str, session: SessionDep):
    ...
    if not work_order:
        raise HTTPException(status_code=404, detail=f"Work order '{base_id}' not found")
...
def get_workorders(
    ...
    if base_id_startswith:
        statement = statement.where(WorkOrder.base_id.startswith(base_id_startswith))
